...
extern int init_client_socket();
extern void send_data(TRANSMIT_DATA *td, char *server_ip, int port_num);
extern int init_server_socket(int port_num);
extern void update_loged_chunks(int given_chunk_id);
...
extern int get_dest_node_id(int updt_chunk_id);
...
extern void listen_ack(TRANSMIT_DATA* cmt_ntf_td, char* recv_buff, int stripe_id, int updt_dt_id, int updt_prty_id, int port_num, int op_type);
...
extern void aggregate_data(char* data_delta, int num_recv_chnks);
extern int get_local_chunk_id(int stripe_id);
...
extern int   nodes_in_racks[3];
extern char* node_ip_set[6];
